...
class GenerateImageArgs(BaseModel):
    prompt: str = Field(description="最终用于 AI 生图或修图的完整提示词，必须包含主体、风格、构图、文字约束。")
...
def create_image_tool(ctx: OptionalToolContext, config: ImageAgentConfig):
    ...
    async def generate_and_send_image(
        ...
        只有用户明确要求调用 AI 生成或修改图片时才调用，例如生成、画、做图、P图、改图、
        编辑图片、风格化、二创、重绘、合成。
        用户只是说“发一张图片 / 来张图 / 给我一张图片 / 发图 / 找张图片 / 看图”，不要调用本工具。
...
async def build(ctx: OptionalToolContext) -> OptionalToolBundle:
  ...
  - 用户只是聊天、评价图片、问你怎么看、让你转述图片内容时，不要调用
