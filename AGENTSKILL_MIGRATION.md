...
当前工作区的全部 6 个工具都将长规则从 Bundle 常驻 `prompt` 移到一个按需 Skill：
...
另外，6 个工具入口均通过 Python 3.12 `py_compile` 检查，动态目录导入仍使用主插件现有 loader。

## 首轮 token 对比

迁移前，6 份完整规则的静态文本约 2,810 字符；迁移后，Bundle 常驻规则为 0 字符，技能索引（含主插件固定索引说明）约 342 字符。按中文约 2.8 字符/token 的粗略估算，首轮规则部分约从 1,004 token 降到 122 token，静态文本减少约 87.8%；这还没有把 6 个业务工具 schema 从首轮解绑带来的节省计算进去。旧值不包含运行时 f-string 展开的动态字段，新值不包含主 Agent 其他系统提示；实际 token 数会随模型 tokenizer 和主插件配置变化。完整规则和 schema 只在对应任务加载 Skill 后出现。
